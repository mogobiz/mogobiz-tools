...
public class Scalr {
  ...
  protected static BufferedImage move(BufferedImage src,
                                            int hPad, int vPad, Color color) {
    // Setup the rendering resources to match the source image's
    BufferedImage result = createOptimalImage(src, src.getWidth(), src.getHeight());
    Graphics2D resultGraphics = result.createGraphics();
    resultGraphics.setColor(color);
    resultGraphics.fillRect(0, 0, src.getWidth(), src.getHeight());
    resultGraphics.drawImage(src, hPad, vPad, src.getWidth(), src.getHeight(), null);
    // Just to be clean, explicitly dispose our temporary graphics object
    resultGraphics.dispose();
